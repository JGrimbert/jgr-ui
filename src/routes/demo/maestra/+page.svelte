<script lang="ts">
  ...
  // ── Formulaire ───────────────────────────────────────────────────────────────
  let listStr  = $state('alpha\nbeta\ngamma')
  let seedStr  = $state('4,4,4,4')
  let kyklos   = $state(3)
  let radius   = $state(100)
  let delta    = $state(0)
  let ellipse  = $state(1)
  let dag      = $state<number[][] | null>(null)
  let arcAngle = $state(150)
  let delta0   = $state(Math.PI / 2)

  function applyPreset(i: number) {
    const p = PRESETS[i].params
    seedStr  = p.seed.join(',')
    kyklos   = p.kyklos   ?? kyklos
    radius   = p.radius   ?? radius
    delta    = p.delta    ?? delta
    ellipse  = p.ellipse  ?? ellipse
    dag      = p.dag      ?? null
    arcAngle = p.arcAngle ?? 150
    delta0   = p.delta0   ?? Math.PI / 2
  }
  ...
  async function run() {
    ...
    try {
      ...
      if (dag) {
        ...
        logs = [
          { line: `params: seed=[${seed}] dag=${JSON.stringify(dag)} arcAngle=${arcAngle}`, type: 'system' },
  ...
  $effect(() => {
    void seedStr; void kyklos; void radius; void delta; void ellipse; void dag
    run()
  })
</script>
...
<div class="page">
  ...
  <aside class="sidebar">
    ...
    <section class="form-section">
      <label class="form-label" for="list">liste</label>
      <textarea id="list" class="form-input form-textarea" rows="4"
                bind:value={listStr} placeholder="un item par ligne"></textarea>
    </section>
