<script lang="ts">
  ...
  // ── Presets ─────────────────────────────────────────────────────────────────
  // Configs extraites de lolve/server/configs.json (format plat, interface lolve)
  const PRESETS = [
    {
      label: 'Basique [4,4,4,4]',
      params: { seed: [4, 4, 4, 4], kyklos: 3, radius: 100, delta: 0, ellipse: 1 },
    },
    {
      label: 'Deuz [3,4,3,4,3]',
      params: { seed: [3, 4, 3, 4, 3], kyklos: 3, radius: 120, delta: 0.54, ellipse: 1 },
    },
    {
      label: 'Triangulaire [3,3,3,3,6]',
      params: { seed: [3, 3, 3, 3, 6], kyklos: 2, radius: 100, delta: 0, ellipse: 1 },
    },
  ]
  ...
  async function run() {
    ...
    error = null
    logs = [{ line: '⚡ init()', type: 'system' }]

    try {
      const { Biblio } = await import('lolve/src/prima/Biblio.js')
      ...
      logs = [
        { line: `params: seed=[${params.seed}] kyklos=${kyklos} radius=${radius}`, type: 'system' },
...
<div class="page">
  ...
  <aside class="sidebar">
    <header class="sidebar-header">
      ...
      <span class="sidebar-sub">jgr-maestra × lolve</span>
    </header>
