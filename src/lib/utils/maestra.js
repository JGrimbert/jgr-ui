 ...
 */
export async function createBiblio(config) {
    const { novaMagma } = await import('lolve/src/prima/Magma.js')
    const { Biblio }    = await import('jgr-maestra/Biblio.js')
    return new Biblio({ novaMagma, ...config }).init({})
}
