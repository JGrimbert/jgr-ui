...
export function computeGravityLevels(nodes, edges) {
  ...
  while (changed) {
    ...
    for (const n of nodes) {
      if (n.isSpine) continue;

      const currentLevel = levelMap.get(n.id) ?? 0;
      const children = childrenOf.get(n.id) ?? [];
      if (children.length === 0) continue;

      const minChildLevel = Math.min(...children.map(c => levelMap.get(c) ?? 0));
      if (minChildLevel < currentLevel + 2) continue;

      // Garde parentale : ne pas remonter au-dessus d'un parent
      const parents = parentsOf.get(n.id) ?? [];
      const maxParentLevel = parents.length > 0
        ? Math.max(...parents.map(p => levelMap.get(p) ?? 0))
        : -1;
      const newLevel = Math.max(maxParentLevel + 1, minChildLevel - 1);

      if (newLevel > currentLevel) {
        levelMap.set(n.id, newLevel);
        changed = true;
      }
    }
  }
